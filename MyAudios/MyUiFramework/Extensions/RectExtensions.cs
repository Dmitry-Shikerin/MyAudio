    ...
    public static class RectExtensions
    {
        ...
        public static Rect ScaleSizeBy(this Rect rect, Vector2 scale, Vector2 pivotPoint)
        {
         ...
         public static Rect Below(this Rect source, Rect belowSource) =>
             new(source.x, belowSource.y + belowSource.height, source.width, source.height);

         public static Rect RightOf(this Rect source, Rect target) =>
             new(target.x + target.width, source.y, source.width, source.height);

         public static Rect WithSize(this Rect source, float width, float height) =>
             new(source.x, source.y, width, height);

         public static Rect WithWidth(this Rect source, float width) =>
             new(source.x, source.y, width, source.height);

         public static Rect WithHeight(this Rect source, float height) =>
             new(source.x, source.y, source.width, height);

         public static Rect Pad(this Rect source, float left, float top, float right, float bottom) =>
             new(source.x + left, source.y + top, source.width - right, source.height - bottom);

         public static Rect PadSides(this Rect source, float padding) =>
             new(source.x + padding, source.y + padding, source.width - padding * 2, source.height - padding * 2);

         public static Rect AlignTopRight(this Rect source, Rect target) =>
             new(target.xMax - source.width, target.y, source.width, source.height);
        ...
        public static Rect LeftHalf(this Rect source) =>
            new(source.x, source.y, source.width / 2, source.height);

        public static Rect RightHalf(this Rect source) =>
            new(source.x + source.width / 2, source.y, source.width / 2, source.height);

        public static Rect TopHalf(this Rect source) =>
            new(source.x, source.y, source.width, source.height / 2);

        public static Rect BottomHalf(this Rect source) =>
            new(source.x, source.y + source.height / 2, source.width, source.height / 2);
        ...
        public static Rect Clip(this Rect source, Rect target)
        {
            ...
            if (source.y > target.yMax)
                y = target.yMax;

            float width = source.width;
            
            if (x + source.width > target.xMax)
                width = target.xMax - source.x;

            float height = source.height;
            
            if (y + source.height > target.yMax)
                height = target.yMax - source.y;

            return new Rect(x, y, width, height);
        }

        public static Rect InnerAlignWithBottomRight(this Rect source, Rect target) =>
            new(target.xMax - source.width, target.yMax - source.height, source.width, source.height);

        public static Rect InnerAlignWithCenterRight(this Rect source, Rect target) =>
            source.InnerAlignWithBottomRight(target).AlignHorizontallyByCenter(target);

        public static Rect InnerAlignWithCenterLeft(this Rect source, Rect target) =>
            source.InnerAlignWithBottomLeft(target).AlignHorizontallyByCenter(target);

        public static Rect InnerAlignWithBottomLeft(this Rect source, Rect target) =>
            new(target.x, target.yMax - source.height, source.width, source.height);
        ...
        public static Rect LeftOf(this Rect source, Rect target) =>
            new(target.x - source.width, source.y, source.width, source.height);

        public static Rect Above(this Rect source, Rect target) =>
            new(source.x, target.y - source.height, source.width, source.height);

        public static Rect AboveAll(this Rect source, Rect target, int i) =>
            new(source.x, target.y - source.height * i, source.width, source.height);
        ...
        public static Rect StretchedVerticallyAlong(this Rect source, Rect target) =>
            new(source.x, source.y, source.width, target.yMax - source.y);

        public static Rect AddHeight(this Rect source, int height) =>
            new(source.x, source.y, source.width, source.height + height);

        public static Rect AddWidth(this Rect source, int width) =>
            new(source.x, source.y, source.width + width, source.height);
    }
}